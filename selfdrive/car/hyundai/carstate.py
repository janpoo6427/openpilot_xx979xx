...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp2, cp_cam):
    ...
    # cruise state
    ret.cruiseState.enabled = bool(cp.vl['EMS16']['CRUISE_LAMP_M'])#(cp_scc.vl["SCC12"]['ACCMode'] != 0) if not self.no_radar else \
    ...
    self.is_set_speed_in_mph = 22 #int(cp.vl["CLU11"]["CF_Clu_SPEED_UNIT"])
    if ret.cruiseState.enabled:
      speed_conv = CV.MPH_TO_MS if self.is_set_speed_in_mph else CV.KPH_TO_MS
      ret.cruiseState.speed = 22 # cp_scc.vl["SCC11"]['VSetDis'] * speed_conv if not self.no_radar else \
